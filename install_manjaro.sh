...
install_python( )
{
  if ! pacman -Qi python | grep 'Installed Size' ; then
    echo -n 'Package python required. Install? (y/n): '
    read install_python
    if [ $install_python = 'y' ]; then
      echo "Installing python.."
      if ! pacman -S python; then
...
install_open_vpn( )
{
  if ! pacman -Qi networkmanager-openvpn | grep 'Installed Size' > /dev/null ; then
    ...
    if [ $install_openvpn = 'y' ]; then
      ...
      error "Package networkmanager-openvpn is required for installation. Aborting.."
    fi
  else
    echo "Package networkmanager-openvpn already installed"
  fi
}
