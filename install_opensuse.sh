...
install_python( )
{
  if ! rpm -qa python > /dev/null ; then
    echo -n 'Python required. Install? (y/n): '
    read install_py
    if [ $install_py = 'y' ]; then
      echo "Installing python.."
      if ! zypper install python; then
        error "Error installing python Aborting.."
      fi
    else
      error "Python is required for installation. Aborting.."
    fi
  else
...
install_open_vpn( )
{
  if ! rpm -qa NetworkManager-openvpn > /dev/null ; then
    echo -n 'Package openvpn required. Install? (y/n): '
    read install_openvpn
    if [ $install_openvpn = 'y' ]; then
      echo "Installing openvpn.."
      if ! zypper install NetworkManager-openvpn; then
        error "Error installing openvpn. Aborting.."
      fi
    else
